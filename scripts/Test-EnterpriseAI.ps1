...
try {
    ...
    Initialize-EAILogging -LogPath (Join-Path -Path $config.ProgramDataRoot -ChildPath 'Logs\Repair\validation.log')
    Write-EAIScriptLog -Message 'Starting EnterpriseAI validation suite.' -Level 'INFO'

    $results = Invoke-EAIValidationSuite -Config $config -PersistReport
    ...
    $exitCode = Get-EAIValidationExitCode -Results $results
    if ($exitCode -eq $script:ExitCodes.Success) {
        Write-EAIScriptLog -Message 'EnterpriseAI validation passed with no FAIL results.' -Level 'INFO'
    }
    else {
        Write-EAIScriptLog -Message 'EnterpriseAI validation found one or more FAIL results.' -Level 'ERROR'
    }
    ...
    if ($script:LogPath) {
        Write-EAIScriptLog -Message ('Unhandled validation error: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }
    else {
