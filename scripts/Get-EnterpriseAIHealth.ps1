...
if (-not $PSBoundParameters.ContainsKey('ConfigPath') -or [string]::IsNullOrWhiteSpace($ConfigPath)) {
    $ConfigPath = Get-EAIDefaultConfigPath
}
...
try {
    ...
    Initialize-EAILogging -LogPath (Join-Path -Path $config.ProgramDataRoot -ChildPath 'Logs\Repair\health.log')

    $checks = New-Object System.Collections.Generic.List[object]

    foreach ($serviceName in @($config.OllamaServiceName, $config.LiteLLMServiceName)) {
        $service = Get-Service -Name $serviceName -ErrorAction SilentlyContinue
        $checks.Add([pscustomobject]@{
            Name    = $serviceName + 'Running'
            Status  = if ($service -and $service.Status -eq 'Running') { 'Healthy' } else { 'Degraded' }
            Details = if ($service) { $service.Status.ToString() } else { 'Missing' }
        })
    }

    $litePing = Invoke-EAIHttpProbe -Uri ($config.LiteLLMEndpoint.TrimEnd('/') + '/v1/models')
    $checks.Add([pscustomobject]@{
        Name    = 'LiteLLMEndpointResponse'
        Status  = if ($litePing.StatusCode -in @(200, 401)) { 'Healthy' } else { 'Degraded' }
        Details = 'HTTP ' + $litePing.StatusCode
    })

    $liteUnauth = Invoke-EAIHttpProbe -Uri ($config.LiteLLMEndpoint.TrimEnd('/') + '/v1/models')
    $checks.Add([pscustomobject]@{
        Name    = 'LiteLLMRejectsUnauthenticated'
        Status  = if ($liteUnauth.StatusCode -eq 401) { 'Healthy' } else { 'Degraded' }
        Details = 'HTTP ' + $liteUnauth.StatusCode
    })

    $ollamaPing = Invoke-EAIHttpProbe -Uri ($config.OllamaEndpoint.TrimEnd('/') + '/api/tags')
    $checks.Add([pscustomobject]@{
        Name    = 'OllamaLocalReachable'
        Status  = if ($ollamaPing.StatusCode -in @(200, 404)) { 'Healthy' } else { 'Degraded' }
        Details = 'HTTP ' + $ollamaPing.StatusCode
    })

    $overall = if ($checks | Where-Object { $_.Status -ne 'Healthy' }) { 'Degraded' } else { 'Healthy' }
    ...
    exit $script:ExitCodes.ValidationFailed
}
catch {
    if ($script:LogPath) {
        Write-EAIScriptLog -Message ('Unhandled health check error: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }
    else {
