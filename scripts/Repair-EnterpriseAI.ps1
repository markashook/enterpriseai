...
try {
    ...
    Initialize-EAILogging -LogPath (Join-Path -Path $config.ProgramDataRoot -ChildPath 'Logs\Repair\repair.log')
    Write-EAIScriptLog -Message 'Starting EnterpriseAI repair.' -Level 'INFO'
    ...
    try {
        ...
        Exit-EAIScript -Code $script:ExitCodes.AclFailed -Message ('ACL repair failed: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }

    try {
        Ensure-EAIServiceAccount -Name $config.ServiceAccountName -ProgramDataRoot $config.ProgramDataRoot -RepairOnly
    }
    catch {
        Exit-EAIScript -Code $script:ExitCodes.ServiceAccountFailed -Message ('Service account repair failed: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }
    ...
    try {
        ...
        if (Test-Path -LiteralPath $secretPath) {
            $liteEnv['LITELLM_MASTER_KEY'] = (Get-Content -Path $secretPath -Raw).Trim()
        }
        ...
        Exit-EAIScript -Code $script:ExitCodes.ServiceFailed -Message ('Service repair failed: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }
    ...
    try {
        ...
        Exit-EAIScript -Code $script:ExitCodes.FirewallFailed -Message ('Firewall repair failed: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }
    ...
    try {
        ...
        Exit-EAIScript -Code $script:ExitCodes.ServiceFailed -Message ('Service restart failed: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }
    ...
    if ($validationResults | Where-Object { $_.Status -eq 'FAIL' }) {
        Exit-EAIScript -Code $script:ExitCodes.ValidationFailed -Message 'Repair validation failed.' -Level 'ERROR'
    }

    Write-EAIRegistryState -Config $config -ConfigPath $ConfigPath
    Write-EAIScriptLog -Message 'EnterpriseAI repair completed successfully.' -Level 'INFO'
    exit $script:ExitCodes.Success
}
catch {
    if ($script:LogPath) {
        Write-EAIScriptLog -Message ('Unhandled repair error: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }
    else {
