...
try {
    ...
    try {
        $config = Read-EAIConfigFile -ConfigPath $ConfigPath -SkipRuntimeChecks
    }
    catch {
    ...
    Initialize-EAILogging -LogPath (Join-Path -Path $config.ProgramDataRoot -ChildPath 'Logs\Uninstall\uninstall.log')
    Write-EAIScriptLog -Message 'Starting EnterpriseAI uninstall.' -Level 'INFO'

    $partial = $false
    $steps = @(
        @{ Action = 'Stop LiteLLM service'; Target = $config.LiteLLMServiceName; Script = { Stop-EAIServiceSafe -Name $config.LiteLLMServiceName } },
        @{ Action = 'Stop Ollama service'; Target = $config.OllamaServiceName; Script = { Stop-EAIServiceSafe -Name $config.OllamaServiceName } },
        @{ Action = 'Remove LiteLLM service'; Target = $config.LiteLLMServiceName; Script = { Remove-EAIWindowsService -Config $config -Name $config.LiteLLMServiceName } },
        @{ Action = 'Remove Ollama service'; Target = $config.OllamaServiceName; Script = { Remove-EAIWindowsService -Config $config -Name $config.OllamaServiceName } },
        @{ Action = 'Remove package-owned firewall rules'; Target = $config.FirewallRulePrefix; Script = { Remove-EAIFirewallRulesByPrefix -Prefix $config.FirewallRulePrefix } },
        @{ Action = 'Remove install root'; Target = $config.InstallRoot; Script = { if (Test-Path -LiteralPath $config.InstallRoot) { Remove-Item -Path $config.InstallRoot -Recurse -Force } } },
        @{ Action = 'Remove program data root'; Target = $config.ProgramDataRoot; Script = { if ($RemoveProgramData -and (Test-Path -LiteralPath $config.ProgramDataRoot)) { Remove-Item -Path $config.ProgramDataRoot -Recurse -Force } } },
        @{ Action = 'Remove or disable service account'; Target = $config.ServiceAccountName; Script = { if ($RemoveServiceAccount) { Remove-EAIServiceAccountInternal -Name $config.ServiceAccountName } else { Disable-EAIServiceAccount -Name $config.ServiceAccountName } } },
        @{ Action = 'Remove registry state'; Target = $config.RegistryKey; Script = { Remove-EAIRegistryState -Config $config } }
    )

    foreach ($step in $steps) {
        try {
            if ($step.Action -eq 'Remove program data root' -and -not $RemoveProgramData) {
                Write-EAIScriptLog -Message 'Preserving ProgramData root by default.' -Level 'INFO'
                continue
            }
            if ($step.Action -eq 'Remove or disable service account' -and -not $RemoveServiceAccount) {
                Write-EAIScriptLog -Message 'Disabling service account by default.' -Level 'INFO'
            }
            ...
            $partial = $true
            Write-EAIScriptLog -Message ('Uninstall step failed ({0}): {1}' -f $step.Action, $_.Exception.Message) -Level 'ERROR'
        }
    }

    if ($partial) {
        Exit-EAIScript -Code $script:ExitCodes.UninstallPartial -Message 'EnterpriseAI uninstall completed with partial failures.' -Level 'WARN'
    }

    Write-EAIScriptLog -Message 'EnterpriseAI uninstall completed successfully.' -Level 'INFO'
    exit $script:ExitCodes.Success
}
catch {
    if ($script:LogPath) {
        Write-EAIScriptLog -Message ('Unhandled uninstall error: {0}' -f $_.Exception.Message) -Level 'ERROR'
    }
    else {
